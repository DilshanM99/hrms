    ...
    <main class="flex-1 overflow-y-auto">
        <div class="container mx-auto px-4 py-8 pb-10">
            <div class="flex justify-between items-center mb-6">
                ...
                <a href="<?= site_url('payroll/create_run') ?>"
            ...
            <!-- Search Form -->
            <form method="get" action="<?php echo site_url('payroll'); ?>" class="flex flex-col md:flex-row gap-2 mb-4">
                ...
                <table class="min-w-full bg-white border border-gray-200 rounded-lg">
                    <thead class="bg-gray-50">
                        <tr>
                            <!-- <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer"
                                ...
                                <span class="ml-1">
                                    <?php if ($sort === 'id'): ?>
                                        <i data-feather="<?php echo $order === 'asc' ? 'chevron-up' : 'chevron-down'; ?>"
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        <?php if (empty($payroll_runs)): ?>
                            <tr>
                                <td colspan="8" class="px-6 py-4 text-center text-gray-500">No payroll runs found.</td>
                            </tr>
                        <?php else: ?>
                            <?php foreach ($payroll_runs as $run): ?>
                                <tr class="hover:bg-gray-50">
                                    <!-- <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900"><?php echo html_escape($run['id']); ?></td> -->
                                    <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                                        <?php echo date('F j, Y', strtotime($run['run_date'])); ?>
                                    </td>
                                    <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                                        <?php echo date('M j', strtotime($run['period_start'])) . ' - ' . date('M j, Y', strtotime($run['period_end'])); ?>
                                    </td>
                                    <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                                        <?php echo ucfirst($run['status']); ?>
                                    </td>
                                    <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                                        Rs <?php echo number_format($run['total_gross'], 2); ?></td>
                                    <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                                        Rs <?php echo number_format($run['total_deductions'], 2); ?></td>
                                    <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                                        Rs <?php echo number_format($run['total_net'], 2); ?></td>
                                    <td class="px-4 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                        <a href="<?php echo site_url('payroll/view_run/' . $run['id']); ?>"
                                        ...
                                        <?php if ($run['status'] === 'draft'): ?>
                                            <a href="<?php echo site_url('payroll/edit_run/' . $run['id']); ?>"
                                                class="text-indigo-600 hover:text-indigo-900">
                                            ...
                                            <a href="<?php echo site_url('payroll/delete_run/' . $run['id']); ?>"
                                            ...
                                            <a href="<?php echo site_url('payroll/generate_run/' . $run['id']); ?>"
                                                class="text-green-600 hover:text-green-900"
                                                onclick="return confirm('Generate payroll run?')">
