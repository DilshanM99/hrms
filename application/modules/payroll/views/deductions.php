...
<div class="flex-1 flex flex-col overflow-hidden">
    <main class="flex-1 overflow-y-auto">
        <div class="container mx-auto px-4 py-8 pb-10">
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-2xl font-bold text-gray-900">Deductions Management</h1>
                <a href="<?php echo site_url('payroll/create_deduction'); ?>" class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white rounded-md hover:bg-indigo-700">
            ...
            <!-- Search Form -->
            <form method="get" action="<?php echo site_url('payroll/deductions'); ?>" class="flex flex-col md:flex-row gap-2 mb-4">
                <?php echo form_hidden($csrf['name'], $csrf['hash']); ?>
                ...
                <button type="button" onclick="window.location.href='<?php echo site_url('payroll/deductions'); ?>'" class="bg-gray-300 text-gray-700 px-4 py-2 rounded-md hover:bg-gray-400 flex items-center">
                ...
                <table class="min-w-full bg-white border border-gray-200 rounded-lg">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <?php else: ?>
                            <?php foreach ($deductions as $deduction): ?>
                                <tr class="hover:bg-gray-50">
                                    <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900"><?php echo html_escape($deduction['name']); ?></td>
                                    <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900"><?php echo ucfirst($deduction['type']); ?></td>
                                    <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900">Rs <?php echo number_format($deduction['amount'], 2); ?></td>
                                    <td class="px-4 py-4 text-sm text-gray-900">
                                        <?php
                                        if ($deduction['type'] === 'variable' && !empty($deduction['months'])) {
                                            $month_names = array_map(function($month) {
                                                return date('M', mktime(0, 0, 0, $month, 1));
                                            }, $deduction['months']);
                                            echo count($month_names) === 12 ? 'All Months' : implode(', ', $month_names);
                                            if (!empty($deduction['year'])) {
                                                echo ' (' . $deduction['year'] . ')';
                                            }
                                        } else {
                                            echo '-';
                                    ...
                                    <td class="px-4 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                        <a href="<?php echo site_url('payroll/view_deduction/' . $deduction['id']); ?>" class="text-blue-600 hover:text-blue-900" title="View">
                                            <svg class="w-4 h-4 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/>
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/>
                                            </svg>
                                        </a>
                                        <a href="<?php echo site_url('payroll/edit_deduction/' . $deduction['id']); ?>" class="text-indigo-600 hover:text-indigo-900" title="Edit">
                                            <svg class="w-4 h-4 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                            </svg>
                                        </a>
                                        <a href="<?php echo site_url('payroll/delete_deduction/' . $deduction['id']); ?>" class="text-red-600 hover:text-red-900" title="Delete" onclick="return confirm('Are you sure you want to delete this deduction?')">
                                            <svg class="w-4 h-4 inline" fill="none" stroke="currentColor" viewBox="0 0 24 24">
