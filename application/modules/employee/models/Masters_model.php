...
class Masters_model extends CI_Model
{
    ...
    public function get_employee_departments($employee_id)
    {
        ...
        $this->db->join('employee_departments ed', 'd.id = ed.department_id');
        $this->db->where('ed.employee_id', $employee_id);
        return $this->db->get()->result_array();
    }
    ...
    public function update_employee_department($employee_id, $department_id, $assigned_date = NULL)
    {
        ...
        $data = [
            'employee_id' => $employee_id,
            'department_id' => $department_id,
            'created_at' => $assigned_date ?: date('Y-m-d H:i:s'),
            'updated_at' => date('Y-m-d H:i:s')
        ];
        return $this->db->insert('employee_departments', $data);
    }
    ...
    public function get_employee_designations($employee_id)
    {
        ...
        $this->db->join('employee_designations ed', 'd.id = ed.designation_id');
        $this->db->where('ed.employee_id', $employee_id);
        return $this->db->get()->result_array();
    }
    ...
    public function get_all_designations($search = '', $sort = 'id', $order = 'asc')
    {
        $this->db->select('d.id, d.name, d.description, dept.name as department_name');
        $this->db->from('designations d');
        $this->db->join('departments dept', 'd.department_id = dept.id', 'left');
        if ($search) {
            $this->db->group_start();
    ...
    public function get_designation_by_id($id)
    {
        $this->db->select('d.id, d.name, d.description, d.department_id, dept.name as department_name');
        $this->db->from('designations d');
        $this->db->join('departments dept', 'd.department_id = dept.id', 'left');
