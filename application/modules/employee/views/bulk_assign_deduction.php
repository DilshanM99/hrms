...
<div class="container mx-auto px-4 py-8">
    <h1 class="text-2xl font-bold text-gray-900 mb-6">Bulk Assign deduction</h1>
    ...
    <?php echo form_open('employee/bulk_assign_deduction', ['class' => 'space-y-4']); ?>
        ...
        <div>
            ...
            <select name="deduction_id" id="deduction_id" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" required>
                <option value="">Choose an deduction</option>
                <?php foreach ($deductions as $deduction): ?>
                    <option value="<?php echo $deduction['id']; ?>" data-type="<?php echo $deduction['type']; ?>"><?php echo html_escape($deduction['name']); ?> (<?php echo ucfirst($deduction['type']); ?>)</option>
                <?php endforeach; ?>
            </select>
        </div>
    ...
    <main class="flex-1 overflow-y-auto">
        <div class="container mx-auto px-4 py-8 pb-10">
            <div class="flex justify-between items-center mb-6">
                <h1 class="text-2xl font-bold text-gray-900">Bulk Assign Deductions</h1>
                <a href="<?php echo site_url('employee'); ?>"
                    class="inline-flex items-center px-4 py-2 bg-gray-300 text-gray-700 rounded-md hover:bg-gray-400 transition-colors duration-200"
            ...
            <?php echo form_open('employee/store_bulk_assign_deduction', ['class' => 'space-y-6']); ?>
            <?php echo form_hidden($csrf['name'], $csrf['hash']); ?>

            <div>
                <label for="deduction_ids" class="block text-sm font-medium text-gray-700 mb-1">Deductions</label>
                <div class="flex items-center space-x-4">
                    <select name="deduction_ids[]" id="deduction_ids" multiple
                        class="select2 w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                        required aria-label="Select deductions">
                        <?php foreach ($deductions as $deduction): ?>
                            <option value="<?php echo $deduction['id']; ?>" <?php echo set_select('deduction_ids[]', $deduction['id']); ?>><?php echo html_escape($deduction['name']); ?></option>
                        <?php endforeach; ?>
                    </select>
                    <div class="flex space-x-2">
                        <button type="button" id="select-all-deductions"
                            class="inline-flex items-center px-3 py-1 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition-colors duration-200 min-w-max"
                            title="Select all deductions">
                            Select All
                        </button>
                        <button type="button" id="clear-deductions"
                            class="inline-flex items-center px-3 py-1 bg-gray-300 text-gray-700 rounded-md hover:bg-gray-400 transition-colors duration-200"
                            title="Clear all deductions">
                            Clear
                        </button>
                    </div>
                </div>
                <?php echo form_error('deduction_ids[]', '<p class="text-red-500 text-sm mt-1">', '</p>'); ?>
            </div>
            ...
            <div>
                ...
                <div class="flex items-center space-x-4">
                    ...
                    <div class="flex space-x-2">
                        <button type="button" id="select-all-departments"
                            class="inline-flex items-center px-3 py-1 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition-colors duration-200 min-w-max"
                            title="Select all departments">
                            Select All
                        </button>
                        <button type="button" id="clear-departments"
                            class="inline-flex items-center px-3 py-1 bg-gray-300 text-gray-700 rounded-md hover:bg-gray-400 transition-colors duration-200"
            ...
            <div>
                ...
                <div class="flex items-center space-x-4">
                    <select name="designation_ids[]" id="designation_ids" multiple
                        class="select2 w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
                        aria-label="Select Designations">
                        <?php foreach ($designations as $designation): ?>
                            <option value="<?php echo $designation['id']; ?>"
                                data-department-id="<?php echo $designation['department_id']; ?>" <?php echo set_select('designation_ids[]', $designation['id']); ?>>
                                <?php echo html_escape($designation['name']); ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                    <div class="flex space-x-2">
                        <button type="button" id="select-all-designations"
                            class="inline-flex items-center px-3 py-1 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition-colors duration-200 min-w-max"
                            title="Select all designations">
                            Select All
                        </button>
                        <button type="button" id="clear-designations"
                            class="inline-flex items-center px-3 py-1 bg-gray-300 text-gray-700 rounded-md hover:bg-gray-400 transition-colors duration-200"
                            title="Clear all designations">
                            Clear
                        </button>
                    </div>
                </div>
            ...
            <div>
                ...
                <div class="flex items-center space-x-4">
                    <select name="employee_ids[]" id="employee_ids" multiple
                        ...
                        <?php foreach ($employees as $employee): ?>
                            <option value="<?php echo $employee['id']; ?>" <?php echo set_select('employee_ids[]', $employee['id']); ?>>
                                <?php echo html_escape($employee['first_name'] . ' ' . $employee['last_name']); ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                    <div class="flex space-x-2">
                        <button type="button" id="select-all-employees"
                            class="inline-flex items-center px-3 py-1 bg-indigo-600 text-white rounded-md hover:bg-indigo-700 transition-colors duration-200 min-w-max"
                            title="Select all employees">
                            Select All
                        </button>
                        <button type="button" id="clear-employees"
                            class="inline-flex items-center px-3 py-1 bg-gray-300 text-gray-700 rounded-md hover:bg-gray-400 transition-colors duration-200"
            ...
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Deduction Details</label>
                <div class="flex flex-col md:flex-row md:space-x-4 space-y-4 md:space-y-0">
                    <div class="flex-1">
                        <label for="amount" class="block text-sm font-medium text-gray-700 mb-1">Amount</label>
                        <input type="number" name="amount" id="amount" value="<?php echo set_value('amount'); ?>"
                    ...
                    <div class="flex-1">
                        ...
                        <input type="date" name="expire_date" id="expire_date"
                            value="<?php echo set_value('expire_date'); ?>"
                            class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm"
            ...
            <div class="flex space-x-4">
                <button type="submit"
                    class="bg-indigo-600 text-white px-4 py-2 rounded-md hover:bg-indigo-700 flex items-center transition-colors duration-200"
                    title="Assign selected deductions">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <a href="<?php echo site_url('employee'); ?>"
                    class="bg-gray-300 text-gray-700 px-4 py-2 rounded-md hover:bg-gray-400 flex items-center transition-colors duration-200"
                    title="Cancel and return to employees">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12">
                        </path>
                    </svg>
...
<script>
    $(document).ready(function () {
        ...
        $('#deduction_ids, #department_ids, #designation_ids, #employee_ids').select2({
        ...
        // Select All for deductions
        $('#select-all-deductions').click(function () {
            $('#deduction_ids').find('option').prop('selected', true);
            $('#deduction_ids').trigger('change');
        });

        // Clear deductions
        $('#clear-deductions').click(function () {
            $('#deduction_ids').val(null).trigger('change');
        });
